    ...
    BatchEnrollment = (function() {
        ...
        batchEnrollment.prototype.display_response = function(dataFromServer) {
            ...
            if (allowed.length && emailStudents) {
                // Translators: A list of users appears after this sentence;
                renderList(gettext('Email has been sent to the following users via batch enrollment. They will be allowed to enroll once they register:'), (function() { // eslint-disable-line max-len
                    var k, len2, results;
                    results = [];
            ...
            if (autoenrolled.length && emailStudents) {
                // Translators: A list of users appears after this sentence;
                renderList(gettext('Email has been sent to the following users via batch enrollment. They will be enrolled once they register:'), (function() { // eslint-disable-line max-len
                    var k, len2, results;
                    results = [];
