...
(function(define) {
    ...
    define([
        ...
        function($, _, gettext, Backbone) {
            var DOM_SELECTORS = {
                bulk_exception: '.bulk-black-list-exception',
                upload_csv_button: '.upload-blacklist-csv-button',
                browse_file: '.browse-file',
                bulk_black_list_exception_form: 'form#bulk_black_list_exception_form'
            };
            ...
            var MESSAGE_GROUP = {
                ...
                user_already_black_listed: 'user-already-black-listed',
            ...
            return Backbone.View.extend({
                ...
                events: {
                    'change #browseBtn-bulk-blacklist-csv': 'chooseFile',
                    'click .upload-blacklist-csv-button': 'uploadCSV',
                    'click .arrow': 'toggleMessageDetails'
                },

                initialize: function(options) {
                    // Re-render the view when an item is added to the collection
                    this.generate_bulk_certificate_blacklist_url = options.generate_bulk_certificate_blacklist_url;
                },

                render: function() {
                    var template = this.loadTemplate('certificate-bulk-black-list');
                ...
                uploadCSV: function() {
                    var form = this.$el.find(DOM_SELECTORS.bulk_black_list_exception_form);
                    var self = this;
                    form.unbind('submit').submit(function(e) {
                        ...
                        $.ajax({
                            dataType: 'json',
                            type: 'POST',
                            url: self.generate_bulk_certificate_blacklist_url,
                            data: data,
                            processData: false,
                            contentType: false,
                ...
                display_response: function(data_from_server) {
                    $('.bulk-blacklist-results').removeClass('hidden').empty();
                    ...
                    function generate_div(group, heading, display_data) {
                        ...
                        $('<div/>', {
                            class: 'message ' + group
                        }).appendTo('.bulk-blacklist-results').prepend(
                    ...
                    function get_text(qty, group) {
                        ...
                        case MESSAGE_GROUP.successfully_added:
                            text = qty > 1 ? gettext(qty + ' learners are successfully added to black list') :
                                    gettext(qty + ' learner is successfully added to the blacklist');
                            break;

                        case MESSAGE_GROUP.data_format_error:
                            text = qty > 1 ? gettext(qty + ' records are not in correct format and not added to' +
                                    ' the blacklist') :
                                    gettext(qty + ' record is not in correct format and not added to the black' +
                                        ' list');
                            break;
                        ...
                        case MESSAGE_GROUP.user_not_exist:
                            text = qty > 1 ? gettext(qty + ' learners do not exist in LMS and not added to the' +
                                    ' blacklist') :
                                    gettext(qty + ' learner does not exist in LMS and not added to the black list');
                            break;

                        case MESSAGE_GROUP.user_already_black_listed:
                            text = qty > 1 ? gettext(qty + ' learners are already blacklisted and not added to' +
                                    ' the blacklist') :
                                    gettext(qty + ' learner is already black listed and not added to the black ' +
                                        'list');
                            break;
                        case MESSAGE_GROUP.user_already_white_listed:
                            text = qty > 1 ? gettext(qty + ' learners are already   whitelisted and not added to' +
                                    ' the blacklist') :
                                    gettext(qty + ' learner is already whitelisted and not added to the black ' +
                                        'list');
                            break;
                        ...
                        case MESSAGE_GROUP.user_not_enrolled:
                            text = qty > 1 ? gettext(qty + ' learners are not enrolled in course and not added to' +
                                    ' the black list') :
                                    gettext(qty + ' learner is not enrolled in course and not added to the black' +
                                        ' list');
                            break;
                        }
                        return text;
