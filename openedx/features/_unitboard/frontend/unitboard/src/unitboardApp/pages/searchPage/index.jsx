...
const SearchPageContainer = ({my_discovery_url}) => {
  ...
  const getSearchResult = (subject, program_degree_group, learningType, query, school, center, course_recog, course_state, language, size) => {
    ...
    var course_api_url = `${my_discovery_url}/api/v1/lms-search/get-search/?content_type=course&${course_type_query}&q=${query}&course_school=${school}&course_center=${center}&course_recognition=${course_recog}&course_state=${course_state}&course_language=${language}&size=${size}`
  ...
  if(learningType &&  learningType !== undefined && (learningType == "program" || learningType == "all" || learningType == "program-degree")){
    ...
    var program_api_url = `${my_discovery_url}/api/v1/lms-search/get-search/?program_or_degree=program${program_type_query}&q=${query}&program_school=${school}&program_center=${center}&program_course_recog=${course_recog}&program_course_state=${course_state}&program_language=${language}&size=${size}`
  ...
  if(learningType && learningType !== undefined && (learningType == "degree" || learningType == "all" || learningType == "program-degree") ){
    ...
    var degree_api_url = `${my_discovery_url}/api/v1/lms-search/get-search/?program_or_degree=degree${degree_type_query}&q=${query}&program_school=${school}&program_center=${center}&program_course_recog=${course_recog}&program_course_state=${course_state}&program_language=${language}&size=${size}`
...
//  function getSearchData(subject, program_group, learning_type, query="", school, center, course_recog, course_state, language, size= 0) {
const getSearchData = (subject, program_group, learning_type, query="", school, center, course_recog, course_state, language, size= 0) => {
  setCourseResults('')
  setProgramResults('')
  setDegreeResults('')
  setresultLoader()
  getSearchResult(subject, program_group, learning_type, query, school, center, course_recog, course_state, language, size)
  ...
  useEffect(() => {
    ...
    var course_recog = query.get('course_recognition') || ""
    ...
    .then((data) => {
        ...
        getSearchResult(q_subject, q_prg_group, q_learningtype, query_search, q_school, q_center, q_course_recog, q_course_state, selected_language, size)
  ...
  const getCoursePaginationData = (page) => {
  ...
  const getProgramPaginationData = (page) => {
  ...
  const getDegreePaginationData = (page) => {
